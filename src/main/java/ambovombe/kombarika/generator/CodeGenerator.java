...
public class CodeGenerator {
    ...
    public void generateRepositoryFile(
        ...
        if (languageProperties.getFrameworks().get(framework).getRepository().equals(""))
            return;
        ...
        FileUtility.generateFile(path,
                GeneratorService.getFileName(
                        table + languageProperties.getFrameworks().get(framework).getRepositoryProperty().getName(),
    ...
    /**
     * eg : generate -p path -t table1, table2, table3 -package name -l
     * java:spring-boot
     * 
     * @author rakharrs
     */
    ...
    public void generateControllerFile(
        ...
        path = path + File.separator + directory;
        FileUtility.generateFile(path,
                GeneratorService.getFileName(table + this.getFrameworkProperties().getControllerProperty().getName(),
                        languageProperties.getExtension()),
                content);
    }
    ...
    public void generateView(
        ...
        FileUtility.createDirectory(directory, path);
        path = path + File.separator + directory;
        String fileName = GeneratorService.getFileName(table,
                this.getViewDetails().getViews().get(viewType).getExtension());
    ...
    public void generateAllController(
            ...
            String framework) throws Exception {
        for (String table : tables) {
            generateController(path, table, packageName + "." + controller, packageName + "." + repository,
                    packageName + "." + entity, framework);
        }
    }
    ...
    public void generateAllRepository(
        ...
        if (this.getFrameworkProperties().isOneRepository()) {
            generateRepository(path, tables, ObjectUtility.capitalize(repository), packageName + "." + repository,
                    entity, framework);
        } else {
            for (String table : tables) {
