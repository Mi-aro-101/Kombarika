...
public class Entity {
    ...
    public String getEntityClass(String table) {
        ...
        if (!this.getAnnotationProperty().getEntity().equals(""))
            res += this.getLanguageProperties().getAnnotationSyntax().replace("?",
                    this.getAnnotationProperty().getEntity()) + "\n";
        res += this.getLanguageProperties().getAnnotationSyntax().replace("?", this.getAnnotationProperty().getTable())
                .replace("?", table) + "\n"
    ...
    public String getConstructor(String table) {
        String res = "";
        res = "\t"
                + this.getLanguageProperties().getConstructorSyntax().replace("?",
                        ObjectUtility.capitalize(ObjectUtility.formatToCamelCase(table)));
        return res;
    }
    ...
    public String getEntityField(HashMap<String, String> columns, HashMap<String, String> foreignKeys,
        ...
        for (Map.Entry<String, String> set : columns.entrySet()) {
            if (primaryKeys.contains(set.getKey())) {
                res += "\t"
                        + this.getLanguageProperties().getAnnotationSyntax().replace("?",
                                this.getAnnotationProperty().getConstraints().getPrimaryKey())
                        + "\n";
                if (set.getValue().equals("Integer") && !this.getAnnotationProperty().getAutoIncrement().equals("")) {
                    res += "\t"
                            + this.getLanguageProperties().getAnnotationSyntax().replace("?",
                                    this.getAnnotationProperty().getAutoIncrement())
                            + "\n";
            ...
            if (temp != null) {
                res += "\t"
                        + this.getLanguageProperties().getAnnotationSyntax().replace("?",
                                this.getAnnotationProperty().getConstraints().getForeignKey().getAnnotation()
                                        .replace("?", set.getKey()))
                        + "\n";
                if (!this.getAnnotationProperty().getConstraints().getForeignKey().getManyToOne().equals("")) {
                    res += "\t"
                            + this.getLanguageProperties().getAnnotationSyntax().replace("?",
                                    this.getAnnotationProperty().getConstraints().getForeignKey().getManyToOne())
                            + "\n";
                }
                res += "\t"
            ...
            res += "\t"
                    + this.getLanguageProperties().getAnnotationSyntax()
                            .replace("?", this.getAnnotationProperty().getColumn()).replace("?", set.getKey())
                    + "\n";
    ...
    public String generateEntity(DbConnection dbConnection, String template, String table, String packageName)
            throws Exception {
        ...
        String res = template
                .replace("#package#", GeneratorService.getPackage(this.getLanguageProperties(), packageName))
